// ultraviplet Proxy javascript object rewriter.
// Rewrites functions that makes HTTP or Websocket requests, and DOM element selectors & creators.

// ultravioley configurations retrieved from attribute.

var ultraviolet = JSON.parse(atob(document.currentScript.getAttribute('data-config')));
ultraviolet.url = new URL(ultraviolet.url)

// "document.location" and "window.location" are rewritten server-side and replaced with this object.
window.ultravioletLocation = new Proxy({}, {
    set(obj, prop, value) {

        if (prop == 'assign' || prop == 'reload' || prop == 'replace' || prop == 'toString') return;

        console.log(proxify.url(ultraviolet.url.href.replace(ultraviolet.url[prop], value)));

        console.log((ultraviolet.url.href.replace(ultraviolet.url[prop], value)));


        return location[prop] = proxify.url(ultraviolet.url.href.replace(ultraviolet.url[prop], value));
    },
    get(obj, prop) {
        // Had to be done in order to fix Discord.
        if (ultraviolet.url.origin == atob('aHR0cHM6Ly9kaXNjb3JkLmNvbQ==') && ultraviolet.url.pathname == '/app') return window.location[prop];

        if (prop == 'assign' || prop == 'reload' || prop == 'replace' || prop == 'toString') return {
            assign: arg => window.location.assign(proxify.url(arg)),
            replace: arg => window.location.replace(proxify.url(arg)),
            reload: () => window.location.reload(),
            toString: () => { return ultraviolet.url.href }
        }[prop];
        else return ultraviolet.url[prop];
    }    
});

window.document.ultravioletLocation = window.ultravioletLocation;

Object.defineProperty(document, 'domain', {
    get() {
        return ultraviolet.url.hostname;
    },
    set(value) {
        return value;
    }
...
// Any ultraviolet function that rewrites request URLs go under this object.
var proxify = {
    url: (url, type) => {
        ...
        switch(type) {
            case true:
                proxified = atob(url.replace(ultraviolet.prefix, '').split('_').slice(1).splice(0, 1).join()) + url.split('_').slice(2).join('_');
            break;

            default:

                if (url.match(/^(#|about:|data:|blob:|mailto:|javascript:|{|\*)/) || url.startsWith(ultraviolet.prefix) || url.startsWith(window.location.origin + ultraviolet.prefix)) return url;

                if (url.startsWith(window.location.origin + '/') && !url.startsWith(window.location.origin + ultraviolet.prefix)) url = '/' + url.split('/').splice(3).join('/');

                if (url.startsWith('//')) url = 'http:' + url;
                if (url.startsWith('/') && !url.startsWith(ultraviolet.prefix)) url = ultraviolet.url.origin + url;

                if (url.startsWith('https://') || url.startsWith('http://')) url = new URL(url);
                else url = new URL(ultraviolet.url.href.split('/').slice(0, -1).join('/') + '/' + url);

                proxified = ultraviolet.prefix + '_' + btoa(url.href.split('/').splice(0, 3).join('/')) + '_' + "/" + url.href.split('/').splice(3).join('/');
...
proxify.url_http = url => {
    ...
    else if (ultraviolet.baseURL) {
        if (url.startsWith('/')) return url = proxify.url(ultraviolet.baseURL.split('/').splice(0, 3).join('/') + url);
        else return url = proxify.url(ultraviolet.baseURL.split('/').slice(0, -1).join('/') + '/' + url);
...
window.fetch = function(url, options) {
    ...
    if (url) (url.replace(location.hostname, ultraviolet.url.hostname), url = proxify.url_http(url));
...
window.WebSocket = new Proxy(window.WebSocket, {
    construct(target, args) {
        ...
        args[0] = protocol + location.origin.split('/').splice(2).join('/') + ultraviolet.prefix + '?ws=' + btoa(args[0]) + '&origin=' + btoa(ultraviolet.url.origin);
...
window.History.prototype.pushState = new Proxy(window.History.prototype.pushState, {
    apply(target, thisArg, args) {
        ...
        // Discord support
        if (ultraviolet.url.origin == atob('aHR0cHM6Ly9kaXNjb3JkLmNvbQ==') && args[2] == '/app') {
    ...
    get() {
        var cookie = Object.getOwnPropertyDescriptor(window.Document.prototype, 'cookie').get.call(this),
            ...
            cookie_array.forEach(cookie => {
                ...
                if (ultraviolet.url.hostname.includes(cookie_name.split('@').splice(1).join())) new_cookie.push(cookie_name.split('@').splice(0, 1).join() + '=' + cookie_value);
